...
def aggiungi_spiritello(user_id, username, spiritello, variante):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO collezione (user_id, username, spiritello, variante)
        VALUES (%s, %s, %s, %s)
        ON CONFLICT (user_id, spiritello, variante) DO NOTHING
...
def elimina_spiritello(user_id, spiritello, variante):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("DELETE FROM collezione WHERE user_id = %s AND spiritello = %s AND variante = %s",
...
async def rispondi_comando_start(chat_id, message_id):
    ...
    testo_risposta = "✨ <b>SpriteBot 2.0</b> ✨\n\nApri la mini app per gestire la tua collezione di Spiritelli!"
    testo_bottone = "🎮 Apri SpriteBot 2.0"
